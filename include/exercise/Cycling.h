...
// 计数语义：WakeAI 默认每次左右腿稳定相位切换算 1 次。
enum class CyclingCountMode {
    FullCycle,  // A -> B -> A = 1
    EachPedal   // A -> B = 1（更适合闹钟挑战的实时反馈）
};
...
public:
    ...
    // 防止自动标定过早导致阈值过小。
    // 本项目当前 SHIN_SCALE 建议至少 0.10。
    void setSignalTriggerFloor(float scaleTriggerFloor,
                               float kneeYTriggerFloor);

    // 两次计数之间至少间隔多少帧，避免同一次伸腿过程中抖动多计。
...
private:
    ...
    int lastCountFrame_ = -100000;

    CyclingCountMode countMode_ = CyclingCountMode::EachPedal;
    ...
    // V3：给自动阈值设置绝对下限。
    // debug_log(2) 中旧标定得到 trigger=0.035，远小于真实动作振幅，
    // 会把过渡抖动误认为左右相位切换。
    float kMinScaleTrigger_ = 0.10f;
    float kMinKneeYTrigger_ = 0.12f;

    // V3：即使相位偶发抖动，也不允许短时间内连续加分。
    int kMinCountIntervalFrames_ = 10;
